...
`frontend/src/customSkin.ts` 实现 `workmode-skin/v1` 本地声明式导入。浏览器原生文件选择器只读取用户主动选择、最大 32 KB 的 JSON；解析器拒绝未知字段，只输出有限 CSS 变量和结构枚举。`main.tsx` 在首次 render 前恢复已启用的清洗状态，`App.tsx` 负责后续启停与 `workmode-public-custom-skin-v1` 持久化。可选 `chrome.type=hud` 只能使用受维护的 Neon 结构基座，并允许有限长度的显示标签以及 `stepped/continuous` 主舱、`mirrored/continuous` 气泡预设；`SkinChrome.tsx` 和 `NeonHud.tsx` 用这些值渲染同一套只读运行状态。格式不接受 CSS、JavaScript、HTML、URL、静态素材或权限字段，不能成为任意代码入口。详细 schema 和示例见 `docs/CUSTOM-SKINS.md`。
