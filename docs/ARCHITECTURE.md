...
文献智库不复用工作台的首次向导阶段。`frontend/src/literature/onboarding.ts` 用独立的 `workmode-public-literature-onboarding-v1` 状态维护文献界面导览；状态机先根据 `/api/settings` 返回的模型地址与密钥状态决定是否进入配置前置阶段。未配置时，`LiteratureOnboarding.tsx` 用遮罩和高亮框指向文献活动栏的设置按钮，要求用户进入共享设置并按 DeepSeek 新手说明完成配置；从设置返回后重新读取真实配置状态。配置完成后，同一组件不再渲染居中说明弹窗，而是依次读取 `data-literature-guide` 锚点的 DOM 位置，高亮当前项目、导入、分组/标签筛选、文献列表和对话输入区；窗口缩放、内部滚动与 HUD/预设皮肤切换都使用真实锚点重新定位。导览完成或跳过后不再显示，设置页可以单独重置。MinerU 不属于首次导览前置条件，需要复杂版面或扫描件增强时才按真实任务提示。文献页打开共享设置时，工作台 `FirstRunWizard` 与 `GuidedTour` 被抑制，避免旧引导串场。两套引导状态都只在本机 UI 层，不注入模型上下文。每个新文献 session 另由后端持久化一条 assistant 自我介绍，属于正式对话历史而非 UI 引导状态；既有历史 session 不被批量改写。
...
`desktop/src-tauri/src/lib.rs` 负责单实例、托盘、窗口、固定 `127.0.0.1:8765` 后端和退出清理。原生 Word 插件也只调用该固定 loopback API；端口被其它程序占用时桌面端必须明确启动失败，不能悄悄换端口导致 Word 插件连错服务。后端使用 `PYTHONDONTWRITEBYTECODE=1`，避免在安装目录生成运行缓存。
...
`project_tool_schemas(project_slug)` 根据项目类型返回工具画像。普通项目仍得到通用项目、Web、memory 和 plan 工具；文献项目只得到 17 个 `literature_*` 工具，明确不加载 Shell、Python、通用文件、Web、通用 memory 和 plan。`literature_library_overview` 从 `catalog.json`、`groups.json`、`tags.json` 和 `LITERATURE_FIELD_REGISTRY` 返回论文总数、处理状态、元数据完整度、资产登记数、真实文献分组及使用数、活跃彩色标签组及使用数、归档标签/标签组计数和共享字段契约；它明确把文献分组与标签组分开，禁止模型根据 `papers/` 的物理目录猜逻辑分类。只读 `literature_read` 同时接受一个 `paper_id` 或最多 20 个 `paper_ids`，批量读取以最多 3 个线程隔离单篇失败；它不创建解析任务。`literature_search` 支持按文献分组 ID 和标签 ID 过滤，并与 `literature_read(part="record")` 共用同一记录投影：除稳定 ID 外返回书目字段、元数据质量、处理/校验状态、PDF/SI/解析路径，以及解析后的人类可读文献分组和标签名称；仍被论文引用的归档标签会明确带 `archived=true`，不会伪装成活跃标签。`literature_tag_list` 只读投影权威 `tags.json` 标签注册表，默认排除已归档项，也可按需返回完整注册表；它返回真实标签组及其颜色，以及规范标签 ID、名称、别名、组 ID、状态和按 `catalog.json.tag_ids` 计算的使用次数。`literature_tag_manage` 与前端 `/tag-registry/manage` 共用同一执行器，支持标签组的新建、重命名、改色、归档/恢复，以及标签的新建、重命名、别名、跨组移动、候选确认、归档/恢复。归档标签或标签组只写 `archived_at`，不会删除论文记录中的 `tag_ids`；恢复后引用自动重新投影。归档组时只连带归档当时仍活跃的组内标签，恢复组时也只恢复由该次组归档连带处理的标签。`literature_update_record` 的 schema 描述要求写标签前先调用只读注册表并复用已有规范标签，也允许更新 `publication_date`。`literature_delete` 与 `literature_restore` 提供论文级可恢复删除；`literature_note_delete` 把普通 `notes/*.md` 移入 `notes/.trash/` 并拒绝删除固定 README。删除结果通过 `changed_paths` 触发投影刷新。模型产生的文献写工具会在结构校验后直接执行，不存在 proposal、confirm/reject 工具、`confirmed` 字段或确认关键词解析。前端选择的论文和笔记随用户消息写入 `meta.active_context`；论文选择变化还会先写入 `literature_selection_changed` system 事件，Prompt 用其 paper ID 元数据生成紧凑机器引用，历史 UI 显示的正文只保留文件名。选择只提高本轮相关性，任何真实 paper ID 都可被领域工具处理，选择状态不是授权。用户明确要求精读单篇论文时，文献系统 Prompt 将默认回答切换为逐图证据讲解：先读真实正文与图注，再按图和 panel 说明实验、观察、论证作用与局限；证据不足必须明示且不得猜图，MinerU 增强解析需要先取得用户同意。

`literature_project.py` 是固定结构与原子写入边界。`catalog.json`、`groups.json` 和 `tags.json` 是机器权威源；`LITERATURE_FIELD_REGISTRY` 给人类投影和 AI 工具提供同一字段契约，并为每个字段声明可编辑、可搜索、可筛选、前端可见和 AI 可见状态及人类语义。当前对齐的用户字段包括标题、作者、第一作者姓、年份、发表日期、期刊、期刊缩写、DOI、Workmode 文章类型、文献分组、标签、关注点、摘要、元数据来源/完整度/问题、处理/校验状态、标准档名、归档位置和 PDF/SI/MinerU/全文/事实报告路径；元数据来源同时覆盖 `pdf_metadata`、`crossref`、`ai_pdf`、Cite This、layout.json、人工/库导入和待识别。内容哈希与内部时间戳保留为内部字段。前端详情与 AI 记录投影消费同一语义，前端搜索也覆盖 DOI、第一作者姓和期刊缩写。领域工具负责标签注册、记录、笔记、跨文献段、归档和回收同步。论文删除先锁住 catalog、预检所有受控来源与回收目标，再把主 PDF、SI 文件夹和最外层解析目录移动到 `papers/.trash/<时间>--<paper-id>/files/<原相对路径>`，同时保存含完整记录、原列表位置和移动路径的 `manifest.json`；任一步失败即回滚文件与 catalog。恢复同样先检查所有原目标不存在，再按 manifest 原路移动并重建 `processed-index.md`，绝不覆盖。session/JSONL 属于不可变历史，不参与论文删除。`literature_update_record` 先在内存中规范化并校验候选元数据，再提交 catalog/tag 变更；期刊缩写允许带点、空格和常见标点，标准命名时自动收敛为纯字母数字 CamelCase 片段。标准命名校验失败时不写入候选记录或候选标签，避免失败工具调用造成半写入。通用 Markdown 编辑接口在文献项目中只允许修改既有 `notes/*.md`，协议、catalog、分组、标签、索引和论文产物必须经领域服务。PDF 上传流式写入 `papers/unprocessed/pdf/`，校验 `%PDF-` 并按 SHA-256 去重；重复内容不创建第二条记录，文件名碰撞不会覆盖。

`pdf_metadata.py` 是普通 PDF 入库后的轻量书目层，不属于 MinerU 增强流水线。它用 `pypdf` 有界读取 PDF 属性和前两页文本，保留明确标题/作者，解析主题中的期刊与年份，并提取 DOI；核心字段仍不完整时，用 DOI 或标题+可用年份调用 Crossref REST API，标题搜索结果只有在去标签、去空白后的规范标题完全相等时才接受，避免模糊首条污染 catalog。Crossref 只接收 DOI 或标题/年份，不接收 PDF 文件或正文。前两层仍缺少标题、作者、年份或期刊时，才复用现有 OpenAI-compatible 模型调用读取前两页文字；该轻量调用有独立 60 秒上限，模型返回的每个候选必须带逐字证据，证据不在原文中时不得写入。三层都失败不会撤销已完成的文件入库，而是保留已识别字段并把 `metadata_trust=partial`、失败原因和“元数据待补充”写入同一权威记录。上传路由在线程池运行这条同步降级链并把最终 paper 投影直接返回，因此前端无需维护第二套任务状态；重复 PDF 也会复用原记录并重新尝试补齐。`files.py` 的 PDF/图片预览依赖 Starlette `FileResponse` 生成 `inline` 的 RFC 5987 文件名参数，不得用原始 Unicode 文件名手写 Latin-1 响应头。
...
`zotero_import.py` 接受 Zotero 数据目录或其中的 `zotero.sqlite`，以 SQLite `mode=ro` 和 `query_only` 打开原库；数据库被 Zotero 锁定或结构无法识别时明确要求关闭 Zotero，不尝试修改、复制替换或绕过其锁。自动发现复用本地卷枚举并最多并行扫描四个卷，只匹配文件名精确为 `zotero.sqlite` 的数据库，跳过所有 `storage/` 附件树。标准书目项从 `items/itemData/itemDataValues/fields` 读取标题、日期、期刊、期刊缩写与 DOI，从作者关系表按原顺序读取作者；已删除项及 attachment/note/annotation 子项不会变成论文。Collection 保留多重成员关系并按祖先链拍平为“父分组 - 子分组”；标签保留成员关系，彩色标签按 `syncedSettings.tagColors` 进入真实颜色组，只有自动成员关系的未着色标签进入“Zotero 自动标签”，其它未着色标签进入“Zotero 标签”。`storage:` 附件只允许解析到 `storage/<attachment-key>/` 内，绝对路径的 linked file 可以只读复制，其它无法解析或本机尚未下载的附件按缺失处理。每条记录仍扫描全部本地附件，以首个扩展名和 magic bytes 都有效的 PDF 为主论文，其余存在的文件进入独立 SI；没有有效 PDF 的记录单项失败。导入提交和失败回滚沿用 catalog/tags/groups 项目锁与原子写入边界，不调用模型、MinerU 或 Zotero 服务。
...
`word-addin-native/` 定义原生 COM `Workmode` Ribbon 和 WinForms 引用窗口。`Ribbon.xml` 为插入、管理、刷新、参考文献和样式控件复用 Office 内置 `imageMso` 图标，不把自定义位图资源复制进插件。`WorkmodeWordAddin.cs` 实现 `IDTExtensibility2`/`IRibbonExtensibility`，`CitationDialog.cs` 使用单层 Dock/Panel 区域布局；搜索、分页、列表、可选参数和主操作各占固定语义区域，不使用会在系统缩放下互相挤压的嵌套自动网格。文献接口对完整匹配集合计数，并接受 `offset` 与上限 100 的 `limit`；原生窗口固定每页请求 50 篇，显示当前页、总页数和匹配总数，搜索词变化时回到第一页，上一页/下一页继续复用已提交的搜索条件。由于插件运行在 Word 的 DPI 感知进程中，窗口直接读取 Word 主窗口的 `GetDpiForWindow` 结果，并按该比例同步放大窗体、边距、固定行高和控件宽度；WinForms 自身自动缩放关闭，避免字体已经放大但容器仍保持 96 DPI 像素尺寸。窗口先显示，再通过后台任务异步请求 `word_addin.py`，从而让 Word 主线程继续处理后端 PowerShell 发起的 COM 调用。`word_addin.py` 只按当前活动文献项目搜索论文，并把插入、检查、更新、删除、刷新和参考文献请求转交现有 `word_citations.py`；顶部插件不直接写 Word OOXML，也不引入新的 Custom XML 或第二套引用存储。`OfficeInterop.cs` 只声明插件实际使用的 Office Ribbon 与 `IDTExtensibility2` COM 接口、官方 GUID 和 dispatch 成员；`build-native-word-addin.ps1` 使用 Windows 自带 .NET Framework 编译器把这些契约直接编译进 DLL，不从 Office/GAC 查找 PIA，也不下载第三方互操作包。安装脚本检测 Office 位数，在当前用户 `Software\Classes` 与 Word Addins 注册表中登记版本化 CodeBase，卸载脚本精确移除同一注册。终端仍须安装 Windows 桌面版 Word；构建机不需要安装 Office。该实现不支持 macOS 或 Word 网页版。

NSIS 启动的首个 PowerShell 进程可能与 Office 位数不同。安装和卸载脚本不通过 `Start-Process` 二次包装命令行；它们把脚本、DLL 与平台值编码进 UTF-16LE `-EncodedCommand`，并用单引号字面量转义值，再由原生调用运算符直接启动并等待匹配 Office 位数的 PowerShell。这条边界同时覆盖默认产品名中的空格和合法路径中的英文单引号，避免子进程在复制 DLL 或写注册表前把 `-File` 路径截断，也兼容 GitHub Hosted Runner 的非交互服务会话。安装脚本用 .NET `SHA256` API 计算版本目录，不要求子 PowerShell 自动加载 `Get-FileHash` 所属模块。
...
`literature_routes.py` 只提供固定文件到前端模型的投影和明确的桌面集成动作：创建/识别文献项目、列出 catalog/groups/tags/fields/notes、通过同一领域执行器管理标签注册表、EndNote/Zotero 自动发现/预检/导入、重复扫描、上传 PDF、读取事实/PDF、打开 SI 文件夹、插入 Word 引用/参考文献、直接更新或删除笔记、更新记录/跨文献段和执行归档。它不是第二个对话后端。Word 路由是显式用户动作：引用路由先按当前项目读取 paper 记录并构建快照，参考文献路由只扫描当前 Word 文档中的 Workmode 控件；两者都不调用模型，也不修改项目 catalog。`POST /api/work/literature-projects` 的新客户端只传名称；后端通过 `WORKMODE_MANAGED_PROJECTS_DIR` 分配托管目录，默认优先 `D:\workmode`、无 D 盘时使用 `~/workmode`，目录冲突追加数字。旧客户端仍可传 `root_path`，旧注册项目在原路径零复制兼容；进入投影时幂等补齐缺失固定目录和文件，不覆盖权威数据。项目响应派生 `storage_mode=managed|external`，不要求迁移 storage JSON schema。活动前端位于 `frontend/src/literature/`，由 `frontend/vite.config.ts` 与功能大厅、主工作台、`frontend/src/transcription/` 和 `frontend/src/writing/` 一起构建为 `dist/index.html`、`dist/literature/index.html`、`dist/transcription/index.html` 与 `dist/writing/index.html`；Tauri 继续只打包一份 `frontend/dist`。应用级功能大厅只负责进入文献页面；项目新建、切换、重命名和安全移除位于文献页居中项目管理窗口。移除继续复用共享项目软删除，不删除实体目录。完整工作台不再承担跨模块导航。文献页复用工作台的 `activity-bar` / `activity-btn` 和 `activity-navigation` 皮肤槽位，在左侧提供功能大厅与全局设置；顶部 HUD 统一承载模块品牌、当前项目、文献篇数、项目管理和回收站入口，不显示重复的后端连接或特化模式徽章。普通顶部栏直接渲染这些操作，HUD 皮肤通过 `SkinChrome.actions` 注入同一组真实控件，并通过 `onProjectClick` 保证所有顶部项目字段均可打开项目管理。`buttonTooltips.ts` 在文献根节点内为现有和动态新增的按钮统一补齐原生悬浮提示，优先级为显式 `title`/`data-tooltip`、`aria-label`、强调主文案和普通按钮文案；人工 `title` 不会被自动逻辑覆盖。左栏不再渲染重复的“LIBRARY / 文献库”标题或当前项目区块；搜索独占一行，分组、标签与导入组成等高命令栏，PDF、EndNote 和 Zotero 入口共用导入菜单。标签筛选浮层提供“管理标签”入口；管理窗口读取包含归档项的注册表，直接支持标签组名称/颜色、标签名称/别名/归属、候选确认和可恢复归档，成功后同步刷新列表、筛选器和详情所用的活跃注册表。论文卡片投影标题、期刊、年份、固定一行关注点以及截断后的分组/标签摘要，不展示导入时的原始文件名；缺失标题使用显式“未识别标题”，不以文件名冒充元数据。文献详情概览按内容自适应高度，书目信息区只投影标题、作者、期刊、发表时间、DOI、文献分组和彩色标签；标准档名、原始文件名、内部路径、处理阶段、处理错误、MinerU 目录、归档校验和入档流程仍保留在权威记录与 AI 投影中，但不占用用户阅读首屏。活跃未处理文献常驻渲染与书目信息同为 820px 中轴宽度的编辑区，关注点和 AI 摘要使用等高双栏；标签编辑器只消费前端已加载的活跃 `tags.json` 注册表，支持按名称/别名搜索、按彩色标签组浏览、点击添加或移除，并以稳定标签 ID 维护草稿，保存时再投影为领域接口需要的规范名称和组 ID，不创建自由文本候选。窄屏时双栏、搜索栏和标签组行降为单列。不可编辑记录继续渲染摘要和关注点紧凑槽位，空值显示“暂空”；客观事实无内容时不渲染，删除收进更多菜单。项目笔记和项目记忆同样由全屏 backdrop 居中承载；原始 PDF 页签恢复大工作区并直接使用共享 `PdfViewer`，根据 catalog 中受控的 `paths.pdf` 调用通用 `api.mediaUrl()`。页面切换通过同源 URL 完成，动态桌面 API 地址和目标项目 slug 暂存在同一 WebView 的 `sessionStorage`，认证 token 继续读取共享 `localStorage`。拖拽 PDF 只建立前端待确认清单；确认后才上传；上传响应包含已完成的轻量元数据降级结果，但不会自动勾选文献或启动 MinerU/全文事实流水线。EndNote 导入由桌面文件选择器提供 `.enl`/`.enlx` 路径，Zotero 导入由同一桌面能力选择 `zotero.sqlite`；两者都在后端预检成功且用户确认关闭原软件后才复制，不产生聊天请求。SI 路径只能从 catalog 的受控项目记录解析；用户点击“打开 SI 文件夹”后，前端调用本地后端的专用 POST 路由，由后端校验路径仍在项目根目录内、按需创建目录并调用系统文件管理器，因此浏览器启动模式和 Tauri 安装版共享同一行为。Windows 桌面窗口在 `tauri.conf.json` 中固定 `dragDropEnabled=false`，避免 Tauri 原生拖放拦截 WebView 的 HTML5 `FileList`；浏览器源码模式和安装版因此共用同一条前端确认入库链。批次完成后，后端在同一正式 session 中追加 `literature_import_confirmed` system 事件，正文只有“用户刚刚导入了以下文献”及文件名，paper ID 只保存在元数据；Prompt 构建时从当前 catalog 补充紧凑机器引用，不信任前端编写 paper ID。前端在该事件后出现下一条用户消息之前暂不显示它；下一轮开始时后端通过 `system_message` SSE 把事件放到乐观用户消息之前，最终 JSONL 回读后永久保留。项目标题和 token 百分比来自正式项目记录及 `/context` 结果，不使用静态占位或第二套后台任务轮询。归档在移动任何文件前预检 PDF、SI 与解析目录的所有目标位置，避免目标碰撞导致半归档。
