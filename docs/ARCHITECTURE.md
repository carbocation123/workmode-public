...
`word_citations.py` 负责建立版本化 `workmode-citation/v1` 载荷，只复制标题、作者、年份/发表日期、期刊和 DOI，并以 URL-safe Base64 编码后交给 Windows 自动化层。`word_automation.ps1` 是纯 ASCII 脚本，避免 Windows PowerShell 5 在不同系统代码页下误读 UTF-8 中文；错误码由 Python 翻译成人类可读中文。插入端连接正在运行的 Word，不启动或关闭用户文档；每个引用创建一个纯文本 Content Control，标签为 `workmode-citation:<32 位实例 ID>`，对应文档变量 `WORKMODE_CITATION_DATA_<实例 ID>` 保存项目 slug、稳定 paper ID 与元数据快照。编号按控件在文档中的出现顺序计算，同一项目和 paper ID 复用编号。参考文献使用 `workmode-bibliography:<实例 ID>` 富文本控件，当前只生成确定性的内置数字格式；重新插入或生成会刷新全部现存 Workmode 控件，并删除已经没有引用控件指向的 Workmode 文档变量。该数据模型故意使用 Office.js 也能读取的 Content Control 标签，后续正式 Word 任务窗格可以接管同一文档对象，不需要迁移普通文本。当前实现仅支持 Windows 桌面版 Word；不是 CSL 引擎，也不解析 EndNote/Zotero 私有 Word 字段。
