...
`frontend/src/customSkin.ts` 实现 `workmode-skin/v1` 本地声明式导入。浏览器原生文件选择器只读取用户主动选择、最大 32 KB 的 JSON；解析器拒绝未知字段，只输出有限 CSS 变量映射。`main.tsx` 在首次 render 前恢复已启用的清洗状态，`App.tsx` 负责后续启停与 `workmode-public-custom-skin-v1` 持久化。格式不接受 CSS、JavaScript、URL、静态素材或权限字段，不能成为任意代码入口。详细 schema 和示例见 `docs/CUSTOM-SKINS.md`。
