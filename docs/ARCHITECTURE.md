...
皮肤分成两级。普通皮肤只覆盖 `styles.css` 中的语义化设计令牌，不复制布局规则；结构皮肤在 `ThemeDefinition.layout` 声明布局类型，并通过 `SkinChrome.tsx` 的 `SKIN_CHROME_REGISTRY` 挂载一个顶层外壳。外壳只接收标准化的 `SkinRuntimeProps`（项目名称与路径、模型名、生成状态和应用状态），文件树、会话、消息、工具卡、输入区和查看器仍是同一套业务组件。

新增普通皮肤只需在 `theme.ts` 注册稳定 ID、色板和 CSS 令牌；新增结构皮肤再增加一个外壳组件和注册表条目。结构皮肤的细节样式必须用 `data-theme` 作用域限定，不能直接访问模型密钥、任意后端能力或另存一份业务状态。当前 Neon Space Lab 使用真实上下文占用、活动项目、模型连接和生成状态组成透明荧光蓝舰桥 HUD，并以 2px 语义结构线、选择性不规则主舱和镜像对话气泡重绘现有工作区；纹理来自项目内原创 SVG，没有复制或执行参考包中的二进制、脚本、字体和素材。

`frontend/src/customSkin.ts` 实现兼容 `workmode-skin/v1` 的 `workmode-skin/v2` 本地声明式导入。浏览器原生文件选择器只读取用户主动选择、最大 32 KB 的 JSON；解析器按 schema 使用不同顶层白名单，只输出有限 CSS 变量和结构枚举。`main.tsx` 在首次 render 前恢复已启用的清洗状态，`App.tsx` 负责后续启停与 `workmode-public-custom-skin-v1` 持久化；存储键版本与皮肤协议版本相互独立，以便覆盖升级保留已有 v1 皮肤。

v2 的 `material`、`geometry` 和 `decoration` 只能选择维护过的预设及限幅数值。当前 `soft-cream` 由 `styles.css` 消费解析器生成的阴影位移、模糊、透明度、颗粒、按钮深度和圆角变量，`notebook` 只启用内置胶带、标签与横线装饰；皮肤不能传入任何 CSS 值或纹理地址。可选 `chrome.type=hud` 沿用 Neon 结构基座与有限 HUD 文案/几何枚举，`SkinChrome.tsx` 和 `NeonHud.tsx` 仍只读取标准化运行状态。格式不接受 CSS、JavaScript、HTML、URL、静态素材或权限字段，不能成为任意代码入口。详细 schema 和示例见 `docs/CUSTOM-SKINS.md`。
