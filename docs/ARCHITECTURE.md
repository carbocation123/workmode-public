...
新增普通皮肤只需在 `theme.ts` 注册稳定 ID、色板和 CSS 令牌；新增结构皮肤再增加一个外壳组件和注册表条目。结构皮肤的细节样式必须用 `data-theme` 作用域限定，不能直接访问模型密钥、任意后端能力或另存一份业务状态。当前 Neon Space Lab 使用真实上下文占用、活动项目、模型连接和生成状态组成舰桥 HUD，并重绘现有工作区；它只使用项目内原创 React/CSS，没有复制或执行参考 Rainmeter 包中的二进制、脚本、字体和素材。未来若支持用户导入皮肤，导入格式应保持声明式、资源白名单化，不能加载任意 JavaScript。
