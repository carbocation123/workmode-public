...
主题只覆盖 `styles.css` 中的语义化设计令牌，不复制布局规则。实验室、论文纸、深夜观测站、高对比及跟随系统始终可选；Origin Ring 依赖本地 `tutorial_graduate` 成就。降低动效既支持用户显式开关，也尊重操作系统 `prefers-reduced-motion`。高对比主题额外强化键盘焦点和选中边界。
