...
皮肤维护与应用发行是两条独立流水线。七套源码集中在 `skin-library/sources/`，`scripts/build-skin-library.ps1` 只把协议文件、manifest 声明素材和许可证复制到临时净化目录，再调用离线 Ed25519 签名器，输出到被 Git 忽略的 `skin-library/packages/`。桌面构建脚本与 GitHub Actions 不读取皮肤库；公开 Release 只包含应用安装、更新和校验产物，奖励皮肤由维护者另行手动发放。
