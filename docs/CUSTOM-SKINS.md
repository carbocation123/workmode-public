...
皮肤的 `layout.css` 决定这些内容如何排列、占多大空间以及窗口变窄时如何降级；`visual.css` 决定字体、颜色、材质、边框、图标、装饰和动画。基础主题变量、字体与签名资源在功能大厅、工作台、文献智库、会议转写和文章处理五个表面共享，但布局槽位按页面隔离：只针对 `app-shell` 的旧工作台布局不会自动重排其它页面。manifest palette 与 `--skin-color-*` 先进入 `themeContract.css`，转换为稳定的 `--ui-*` 应用语义；会议转写和文章处理默认消费这些共享角色，只有要改变其专属结构或材质时才分别覆盖 `transcription-*` 与 `writing-*` 槽位。HUD 皮肤可以增加自己的顶栏，但不能隐藏文章页承载功能大厅和模型设置的标题栏；核心布局会为两层标题分别保留网格行。现有 `workmode-skin/v3` 包无需重签即可获得颜色和共享顶栏映射，但不能因此宣称已经完成转写页或文章处理页的专属视觉适配。皮肤没有 JavaScript 或 HTML 入口，不能创建项目数据、伪造工具结果或获得文件/网络权限。需要新增真实数据展示时，必须先由 Workmode 核心提供新的语义槽位。
