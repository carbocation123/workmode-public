...
新增或修改文章处理流程时至少覆盖：空输入与 200,000 字上限、缺失模型配置、两种模式的 Prompt 契约、长文分块、Unicode 转换、成功后持久化、失败不落盘、列表不泄露全文、详情读取、可恢复删除、恢复冲突、前端历史选择/重新处理状态、“新任务”清空编辑区但保留模式、共享设置返回来源、Vite 多页面构建和功能大厅四卡统一布局。面向普通用户的行为变化还要同步更新 [ARTICLE-PROCESSING.md](ARTICLE-PROCESSING.md)。
