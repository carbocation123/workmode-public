...
2. 可以选择 `.enl` 或 `.enlx` 把 EndNote 文献库合并进当前项目。导入前会预检并提醒关闭 EndNote；标题、作者、年份、发表日期、期刊、DOI、手工分组、真实彩色标签和附件在本机直接迁移，不调用模型或 MinerU，也不修改原 EndNote 文件。EndNote 分组集会按“分组集 - 手工分组”拍平，智能分组不会导入。
