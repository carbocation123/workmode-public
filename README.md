...
6. 选中一篇论文后说“精读这篇”，AI 会先读取真实正文和图注，再默认按图 1、图 2……讲解每幅图及各 panel 做了什么、观察到什么、如何支撑结论。图表信息不足时会明确说明，不会猜测。
7. 普通阅读优先使用已有解析文本，否则直接读取 PDF 文本层；需要更可靠地恢复复杂图表或版面时，AI 会提示是否使用 MinerU，不会未经同意自动启动耗时解析。
8. 论文记录、主 PDF、SI 和解析产物可以整体移入项目回收站并无覆盖恢复；历史会话不会被改写。
