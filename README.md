...
6. 文献列表、详情页和 AI 工具读取同一份字段契约。标题、作者、第一作者姓、年份/发表日期、期刊/缩写、DOI、文章类型、文献分组、彩色标签、用户关注点、AI 摘要、主 PDF、SI、解析产物和处理/校验状态会保持同一语义；AI 查询文献库结构时会明确区分 `groups.json` 的文献分组与 `tags.json` 的标签组，不会再根据物理文件夹猜分组。
