...
奖励皮肤的布局、视觉和素材可以独立升级：修改 `skin-library/sources/<skin-id>/` 后运行 `scripts/build-skin-library.ps1` 重新签名，导入同一 ID 会覆盖本机旧包，不需要重新编译或安装 Workmode。签名包统一输出到被 Git 忽略的 `skin-library/packages/`，只做本地测试和手动发放。绿磷终端 4.0.2 已统一 Markdown、图标与输入提示的绿色语义，修正文件正文内边距、token 条溢出、AI/YOU 共轨对齐、头部状态拥挤、终端工具状态槽宽度和文件面板拖动。

奶油泡芙 3.2.3 是第二套完成独立布局与视觉迁移的官方皮肤：在亮色柔软面板、厚按钮、项目卡、猫猫 AI 消息头像、薄荷工具卡、奶油输入区和横线纸张查看器之外，还按 demo 的单行结构重建了项目 crumbs 与真实模型/状态/时间胶囊、两级“研究花园”侧栏标题和紧凑聊天顶栏；品牌标志使用皮肤内签名的矢量兔兔头像，分别绘制耳朵、脸部高光、五官和腮红，不再依赖字符脸与复制阴影。静态样稿中的假按钮和虚构状态不会进入正式包。

Neon Ice 3.1.0 使用无切角的 2px 荧光玻璃 HUD：顶部只展示真实项目、模型、生成状态和本机时间，工作区采用透明冷蓝面板、上下文环、仪器工具卡和独立文件纸面；宽屏文件面板仍可拖动，窄窗口自动隐藏查看器。

霜核机能台 3.1.0 使用银白外壳与深色核心工作区：真实状态顶栏以菱形能量核心为视觉锚点，项目柜、仪器工具卡、亮色文件纸面和可拖动查看器各自使用明确材质层级；窄窗口自动收起查看器。

午夜控制台 3.1.0 使用米白老式电台顶栏、深蓝案件柜、珊瑚热线标签和青绿色信号盘：消息以 `OPERATOR / HOTLINE` 区分角色，工具卡采用寻呼机语言，文件查看器采用保密卷宗语言；频率装饰不伪造运行数据。

紫晶星象塔 3.1.0 使用星空、金线、星徽和暗色手稿材质：顶部、星盘上下文、角色铭牌、仪式记录工具卡与档案查看器均只承载真实项目、模型、消息、工具和文件内容。

酒保行动·像素夜班 3.1.2 与其余奖励皮肤一起维护在统一皮肤库中。它使用本地像素字体、洋红/青色夜班配色、完整矩形对话框、角色名牌、横贯聊天头的上下文仪表带、真实工具类型标签和城市剪影；项目名称与路径只保留在最顶层状态栏，避免聊天头重复。上下文容量与 token 数在左侧成组、历史条数居中、分段预算条与百分比在右侧成组。所有签名包都只做本地测试和手动发放，不作为 GitHub Release 附件。
