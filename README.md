...
7. 左侧列表使用统一的搜索与操作栏：分组、标签和导入保持同一层级，PDF 与 EndNote 入口收在同一个导入菜单中。卡片默认只显示标题、期刊、年份和少量分组/标签，不再重复显示原始文件名。打开文献后默认只显示标题、作者、期刊、日期、DOI、分组和标签；内部路径、标准文件名、处理阶段和归档流程不占用阅读页面。摘要和关注点始终保留紧凑位置，无内容时显示“暂空”；客观事实只在有内容时出现，标签、关注点和摘要通过“编辑信息”按需修改。用户点击“打开 SI 文件夹”后，由本地后端校验项目内路径并调用系统文件管理器，浏览器启动模式和桌面安装版使用同一行为。
