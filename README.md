...
- 提供实验室、Origin Ring、Neon Space Lab、论文纸、深夜观测站和高对比内置皮肤，并支持导入多个官方 Ed25519 签名的 `.workmode-skin`；签名皮肤可分别控制真实内容槽位的布局与视觉，切换时无需重新安装应用。
...
6. 后续点击左侧设置按钮会打开完整设置页；可以在「外观与皮肤」即时切换内置主题或导入官方签名皮肤，在「新手引导与成就」重新播放引导、查看成就或重置教程清单，在「桌面应用」检查更新，在「问题反馈」关注公众号后私信或发送邮件。
...
Workmode 0.7.0 起采用官方签名皮肤协议。每个 `.workmode-skin` 同时包含 `layout.css` 与 `visual.css`：前者只重新排列应用维护的项目、文件、会话、消息、工具、输入区、查看器和设置等真实语义槽位，后者负责颜色、字体、材质、边框、图标、背景和动画。应用会在读取 CSS 前核对内置公钥签名、完整文件清单、大小和 SHA-256；通过后才写入 version 4 本地皮肤库与 IndexedDB。旧 `.workmode-skin.json`、未签名 ZIP 和旧本地皮肤库不会迁移。

GitHub Release 和桌面安装包只分发应用本体，不附带奖励皮肤。取得 `.workmode-skin` 后可在设置页一次选择多个文件导入并通过下拉菜单切换。若皮肤加载失败，应用会自动回到基础主题；若异常退出，下一次启动会停用未稳定的皮肤。紧急恢复快捷键 `Ctrl+Alt+Shift+R` 只清除皮肤选择并刷新，不会删除项目、会话或模型配置。协议、签名边界和签包方式见 [官方签名皮肤](docs/CUSTOM-SKINS.md)。

奖励皮肤的布局、视觉和素材可以独立升级：修改 `skin-library/sources/<skin-id>/` 后运行 `scripts/build-skin-library.ps1` 重新签名，导入同一 ID 会覆盖本机旧包，不需要重新编译或安装 Workmode。签名包统一输出到被 Git 忽略的 `skin-library/packages/`，只做本地测试和手动发放。绿磷终端 4.1.1 已统一 Markdown 与输入提示的绿色语义，修正文件正文内边距、token 条溢出、AI/YOU 共轨对齐、头部状态拥挤、终端工具状态槽宽度和文件面板拖动，并将项目、配置、目录、文件类型、会话和管理按钮升级为三字母终端设备标识；文件与会话标识现在占用独立固定槽位，不再挤压名称。

奶油泡芙 3.3.2 是第二套完成独立布局与视觉迁移的官方皮肤：在亮色柔软面板、厚按钮、项目卡、猫猫 AI 消息头像、薄荷工具卡、奶油输入区和横线纸张查看器之外，还按 demo 的单行结构重建了项目 crumbs 与真实模型/状态/时间胶囊、两级“研究花园”侧栏标题和紧凑聊天顶栏；品牌标志使用皮肤内签名的矢量兔兔头像，项目、设置、文件类型、会话和按钮改用厚软珐琅徽章，并隐藏底层回退字形，避免一个控件同时出现两套标识。文件与会话徽章使用独立 24px 槽位。静态样稿中的假按钮和虚构状态不会进入正式包。

Neon Ice 3.2.1 使用无切角的 2px 荧光玻璃 HUD：顶部只展示真实项目、模型、生成状态和本机时间，工作区采用透明冷蓝面板、上下文环、仪器工具卡和独立文件纸面；导航、文件与按钮使用带内发光的透明 HUD 线框徽标，并为文件与会话徽章保留独立槽位；宽屏文件面板仍可拖动，窄窗口自动隐藏查看器。

霜核机能台 3.2.2 使用银白外壳与深色核心工作区：真实状态顶栏以菱形能量核心为视觉锚点，项目柜、仪器工具卡、亮色文件纸面和可拖动查看器各自使用明确材质层级；导航、文件与控制按钮使用切面晶体模块，文件与会话模块使用独立槽位；亮色消息纸面中的路径和行内代码使用独立高对比仪器屏标签；窄窗口自动收起查看器。

午夜控制台 3.2.2 使用米白老式电台顶栏、深蓝案件柜、珊瑚热线标签和青绿色信号盘：消息以 `OPERATOR / HOTLINE` 区分角色，工具卡采用寻呼机语言，文件查看器采用保密卷宗语言；项目、设置、文件类型、会话与管理按钮改用案件抽屉、磁带盒和硬件键帽标签，并为文件与会话标签保留独立槽位；米白消息纸面中的路径和行内代码改用高对比信号标签；频率装饰不伪造运行数据。

紫晶星象塔 3.2.1 使用星空、金线、星徽和暗色手稿材质：顶部、星盘上下文、角色铭牌、仪式记录工具卡与档案查看器均只承载真实项目、模型、消息、工具和文件内容；导航、档案类型、会话与控制按钮使用黄铜镶边的星图符记，文件与会话符记占用独立槽位。

酒保行动·像素夜班 3.2.1 与其余奖励皮肤一起维护在统一皮肤库中。它使用本地像素字体、洋红/青色夜班配色、完整矩形对话框、角色名牌、横贯聊天头的上下文仪表带、真实工具类型标签和城市剪影；项目名称与路径只保留在最顶层状态栏，避免聊天头重复。上下文容量与 token 数在左侧成组、历史条数居中、分段预算条与百分比在右侧成组。项目、设置、文件夹、各类科研文件、会话与常用管理按钮均使用酒吧夜班专属 CSS 像素画图标，不依赖系统 Emoji；文件与会话图标采用独立槽位，刷新按钮改为相向循环箭头，移除按钮改为像素废纸篓。所有签名包都只做本地测试和手动发放，不作为 GitHub Release 附件。
...
skin-library/        七套奖励皮肤源码；packages/ 为本地签名包并被 Git 忽略
examples/skins/      旧声明式 recipe 与协议回归样例，不是 0.7.0 导入入口
design/skin-lab/     自制静态视觉样稿与设计回归参考
scripts/             源码启动、验证和桌面发行脚本
docs/                架构、开发、发行与产品路线图
```

开发机上的 `.run/` 只保存 PID、临时发行配置、可重建 smoke 目录和项目自带开发工具；不再用作设计稿、论文或教程历史仓库。不能提交的本地参考材料放在 `local-reference/`，其内容默认被 Git 忽略，正式源码与发行流程不得依赖它。

- [开发环境与测试](docs/DEVELOPMENT.md)
- [当前架构](docs/ARCHITECTURE.md)
- [皮肤实验室迁移矩阵](docs/SKIN-LAB-MATRIX.md)
