...
7. 左侧列表使用统一的搜索与操作栏：分组、标签和导入保持同一层级，PDF 与 EndNote 入口收在同一个导入菜单中。卡片默认只显示标题、期刊、年份和少量分组/标签，不再重复显示原始文件名。打开文献后默认只显示标题、作者、期刊、日期、DOI、分组和标签；内部路径、标准文件名、处理阶段和归档流程不占用阅读页面。摘要、关注点和客观事实只在有内容时出现，标签、关注点和摘要通过“编辑信息”按需修改。
