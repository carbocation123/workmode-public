...
科研工作台与文献智库共享同一套项目注册、会话历史、模型配置和上下文机制。会议转写是无 session 的独立文件工具，只读取固定 `input/` 与 `output/`；即使同一目录后来由通用工作台打开并增加其它文件，也不会改变转写列表或任务状态。
...
1. 在共享设置页保存 DashScope API Key；密钥不会回显，也不会写入录音目录或转写结果。
...
2. 安装并启动 Workmode Public，在功能大厅选择“科研工作台”“文献智库”或“会议录音转文字”。
...
| 模型对话与转写 AI 处理 | 仅在用户发送消息，或在已完成转写上确认“AI 润色/总结”时请求用户配置的模型服务；请求会包含完成当前操作所需的上下文或转写文本 |
