...
- 提供实验室、Origin Ring、Neon Space Lab、论文纸、深夜观测站和高对比皮肤，可跟随系统主题并独立降低动效；Neon Space Lab 以透明荧光蓝 HUD 呈现真实项目、上下文、工具与生成状态；设置页还可导入安全的本地声明式皮肤 JSON，包括 HUD 结构与 v2 材质、几何和装饰预设。
...
6. 后续点击左侧设置按钮会打开完整设置页；可以在「外观与皮肤」即时切换主题或导入本地声明式皮肤，在「新手引导与成就」重新播放引导、查看成就或重置教程清单，在「桌面应用」检查更新。
...
本地皮肤使用 `.workmode-skin.json` 声明格式。`workmode-skin/v2` 在兼容 v1 的基础上增加安全材质引擎：可限幅控制面板厚度、内高光、颗粒、按钮下沿、组件圆角和手账装饰。导入器不接受 CSS、JavaScript、HTML、URL、图片、脚本或权限声明；所有阴影和纹理由应用根据枚举与数值生成。用户可以在完整设置页导入、启用、停用和卸载，详见 [本地声明式皮肤](docs/CUSTOM-SKINS.md)。
