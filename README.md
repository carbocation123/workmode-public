...
- 提供实验室、Origin Ring、Neon Space Lab、论文纸、深夜观测站和高对比皮肤，可跟随系统主题并独立降低动效；Neon Space Lab 以透明荧光蓝 HUD 呈现真实项目、上下文、工具与生成状态；设置页还可导入安全的本地声明式皮肤 JSON。
...
本地皮肤使用 `.workmode-skin.json` 声明格式，只允许白名单颜色与材质参数；导入器不接受 CSS、JavaScript、URL、图片、脚本或权限声明。用户可以在完整设置页导入、启用、停用和卸载，详见 [本地声明式皮肤](docs/CUSTOM-SKINS.md)。
