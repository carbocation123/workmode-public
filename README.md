...
项目工具路径都相对当前项目根目录解析。一次工具调用会在聊天区显示为一张紧凑卡片，运行、完成或失败状态原位更新；如果工具修改了文件，前端会在本轮结束后刷新文件树和当前打开文件。聊天区在读者位于底部附近时自动跟随新消息；主动向上滚动后会暂停跟随并显示「回到最新」。
...
- 生成过程中，发送按钮会变成「停止」。停止后不会继续发起工具轮次；已经生成的部分会保留并标记为「已停止生成」。
