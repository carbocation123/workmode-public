...
| **会议录音转文字** | 只想批量上传录音并集中查看结果的用户 | DashScope Fun-ASR、说话人区分、多文件队列、失败重试、TXT/Markdown/JSON 导出和可恢复删除 |
...
5. 标题可以修改；删除会把对应输入与输出整体移入 `output/.trash/`，可从页面恢复。模块不会创建项目、对话或 JSONL。
6. 第一次进入时会播放三步使用指引；第一步从阿里云账号、实名认证、北京地域开通、API Key 创建选项、一次性密钥保存到免费额度防超额逐项说明，并提供百炼控制台和官方文档直达按钮。右上角“使用指引”或共享设置页都可以重新播放。引导进度只保存在当前 WebView 的本地存储中，不写入转写工作目录。
...
- 会议转写当前处理已有录音文件，不提供实时录音、逐字稿在线校对或 AI 会议总结；说话人编号是单次录音内的匿名编号。
