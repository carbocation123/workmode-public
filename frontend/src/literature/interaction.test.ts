...
describe('literature live interaction contracts', () => {
  ...
  it('keeps a compact aligned editor visible and selects existing tags by clicking', () => {
    ...
    expect(source).not.toContain('detailEditing')
    expect(source).not.toContain('收起编辑')
    ...
    expect(detailOverview).toContain('className="review-confirm-panel"')
    expect(detailOverview).toContain('className="detail-edit-fields"')
    expect(detailOverview).toContain('className="detail-tag-picker"')
    expect(detailOverview).toContain('reviewTagIdsDraft.includes(tag.id)')
    expect(detailOverview).toContain('aria-pressed={selected}')
    expect(detailOverview).toContain('搜索已有标签')
    expect(detailOverview).toContain('直接点击添加或移除')
    expect(detailOverview).not.toContain('placeholder="EPR，氧空位，原位表征"')
    ...
    expect(styles).toContain('.detail-edit-fields')
    expect(styles).toContain('grid-template-columns: repeat(2, minmax(0, 1fr))')
    expect(styles).toContain('.detail-tag-options')
  ...
  it('renders dense paper cards without repeating filenames', () => {
    ...
    expect(source).toContain('className="paper-card-focus"')
    expect(source).toContain("paper.focus || '暂空'")
  ...
  it('inserts linked Word citations and a refreshable bibliography from the paper detail', () => {
    ...
    expect(source).toContain('wordDocuments')
    expect(source).toContain('wordDocumentId')
    expect(source).toContain('document_name')
    expect(source).toContain("const [wordBusy, setWordBusy] = useState<'citation' | 'bibliography' | null>(null)")
    expect(literatureApi).toContain('/word/documents')
    expect(literatureApi).toContain('/word/citations/')
    expect(literatureApi).toContain('/word/bibliography')
    expect(literatureApi).toContain('document_id')
  })

  it('offers a complete Word citation manager for search, grouped insertion and maintenance', () => {
    expect(source).toContain('Word 引用管理')
    expect(source).toContain('citationManagerOpen')
    expect(source).toContain('citationStyleId')
    expect(source).toContain('gb-t-7714-2015-numeric')
    expect(source).toContain('american-chemical-society')
    expect(source).toContain('apa-7th')
    expect(source).toContain('引用前缀')
    expect(source).toContain('引用后缀')
    expect(source).toContain('页码或定位信息')
    expect(source).toContain('隐藏作者')
    expect(source).toContain('插入所选文献')
    expect(source).toContain('当前文档中的 Workmode 引用')
    expect(source).toContain('更新引用')
    expect(source).toContain('移除引用')
    expect(source).toContain('失联')
    expect(literatureApi).toContain('/word/styles')
    expect(literatureApi).toContain('/word/citations/inspect')
    expect(literatureApi).toContain('/word/citations/batch')
    expect(literatureApi).toContain('/word/citations/refresh')
    expect(literatureApi).toContain('/word/citations/update')
    expect(literatureApi).toContain('/word/citations/remove')
  })
})
