...
def _check_cancel(store: LiteratureDemoStore, task_id: str) -> None:
    if store.get_task(task_id).get("cancel_requested"):
        raise PipelineCancelled("用户停止了任务")
...
def _set_stage(
    ...
    *,
    ...
    store.update_task(task_id, status="running", stage=stage, progress=progress)
    store.update_paper(paper_id, status=paper_status, stage=stage, error=None)
...
def _mineru_api_key() -> str:
    return (os.getenv("WORKMODE_MINERU_API_KEY") or os.getenv("MINERU_API_KEY") or "").strip()


def _safe_download_url(url: str) -> str:
    if urlsplit(url).scheme.lower() != "https":
        raise LiteraturePipelineError("MinerU 返回了非 HTTPS 下载地址")
    try:
        validate_public_web_url(url)
    except WebToolError as exc:
        raise LiteraturePipelineError("MinerU 返回的下载地址未通过公网安全校验") from exc
    return url
...
def _run_mineru(store: LiteratureDemoStore, task_id: str, paper_id: str) -> Path:
    ...
    recorded_output = paper.get("mineru_output_path")
    if recorded_output:
        recorded_dir = (store.root / str(recorded_output)).resolve()
        if store.root in recorded_dir.parents and (recorded_dir / "full.md").exists():
            return recorded_dir
    output_dir = pdf_path.parent / "minerU识别结果" / pdf_path.stem
    ...
    payload = {
        ...
        "model_version": os.getenv("WORKMODE_MINERU_MODEL_VERSION", "pipeline"),
        "enable_formula": True,
        "enable_table": True,
        "language": os.getenv("WORKMODE_MINERU_LANGUAGE", "en"),
    }
    ...
    with httpx.Client(timeout=timeout, follow_redirects=False) as client:
        ...
        while time.monotonic() < deadline:
            ...
            time.sleep(4)
        if result is None:
            raise LiteraturePipelineError("MinerU 解析等待超过 20 分钟")
    ...
    zip_path = store.root / f".mineru-{task_id}.zip"
    downloaded = 0
    try:
        with httpx.Client(timeout=httpx.Timeout(300, connect=30), follow_redirects=True) as client:
...
def _collect_page_zero_text(output_dir: Path) -> str:
    ...
    for path in candidates:
        ...
        if chunks:
            return "\n".join(chunks)
    full_md = output_dir / "full.md"
    return full_md.read_text(encoding="utf-8", errors="replace")[:40_000]
...
journal_abbreviation 必须是 CamelCase、无点无空格，例如 JACS、ACSAMI、NatCommun；不确定就返回 null。
...
"""
    ...
    if any(metadata.get(key) in {None, ""} for key in required):
        raise LiteraturePipelineError("首页元数据不完整，需要人工确认后才能标准命名")
    try:
    ...
    if source not in {"cite_this", "layout_json_fallback"}:
        raise LiteraturePipelineError("首页元数据来源不能确认，需要人工确认")
    ...
    if metadata.get("paper_type") not in {"research", "review", "unknown"}:
...
def _extract_facts(output_dir: Path, metadata: dict[str, Any]) -> str:
    ...
    page_index_source = ""
    if content_lists:
        page_index_source = content_lists[0].read_text(encoding="utf-8", errors="replace")
    prompt = f"""根据下列 MinerU Markdown 正文制作客观事实抽取报告。

硬性纪律：
1. 只写原文直接陈述的事实、实测数据、实验条件、原文归属和明确论证链；禁止 AI 推理、延伸或跨文献判断。
2. 所有关键数据、现象、观点、g 值和数值必须附原文出处，格式使用 (Fig.X, p.Y)、(Table.X, p.Y)、(Eq.X, p.Y) 或 (p.Y)。若正文无法支持精确定位，明确标记“需核对 page_idx”，不得编造。
3. 报告必须严格有六个一级/二级 Markdown 段：基本信息；仪器与样品；现象与数据；作者观点；证据汇总；跨文献关系与系列归属。
4. 前五段由抽取器填写。第六段只写：⟨待主对话讨论后增补⟩。不得代填。
5. 现象与数据按技术分类，尽量完整保留实验条件、数值、作者归属和必要原文短引。证据汇总必须用 Markdown 表格。
6. 输出完整 Markdown 报告，不要输出 JSON，不要写过程说明。
...
[MinerU content_list.json；关键引用的 p.Y 以这里的 page_idx 为准]
{page_index_source or '缺失：凡是不能定位的内容必须标记需核对 page_idx'}
"""
    report = _model_completion(
        "你是科研文献的客观事实抽取器。事实与解释严格分层，跨文献判断留给主对话。",
        prompt,
    )
    required_headings = ("基本信息", "仪器与样品", "现象与数据", "作者观点", "证据汇总", "跨文献关系")
...
def run_literature_pipeline(store: LiteratureDemoStore, task_id: str) -> None:
    ...
    try:
        if not _mineru_api_key():
            store.update_task(
                ...
                stage="等待配置 MinerU",
                progress=0,
                error="请设置 WORKMODE_MINERU_API_KEY",
            )
            store.update_paper(paper_id, status="pending", stage="等待配置 MinerU")
            return
        _set_stage(store, task_id, paper_id, stage="MinerU 正在解析正文", progress=10, paper_status="parsing")
        output_dir = _run_mineru(store, task_id, paper_id)
        store.update_paper(
            paper_id,
            mineru_output_path=output_dir.relative_to(store.root).as_posix(),
        )

        _set_stage(store, task_id, paper_id, stage="正在核对首页元数据", progress=55, paper_status="extracting")
        current_paper = store.get_paper(paper_id)
        if current_paper.get("metadata_trust") == "complete":
            metadata = {
                key: current_paper.get(key)
                for key in (
            ...
            paper = store.apply_metadata(paper_id, metadata)

        _set_stage(store, task_id, paper_id, stage="正在抽取客观事实", progress=68, paper_status="extracting")
        moved_output = store.pdf_path(paper_id).parent / "minerU识别结果" / Path(str(paper["archive_filename"])).stem
        if output_dir != moved_output:
            moved_output.parent.mkdir(parents=True, exist_ok=True)
            if moved_output.exists():
                moved_output = moved_output.with_name(f"{moved_output.name}_{paper_id[:8]}")
            shutil.move(str(output_dir), str(moved_output))
            output_dir = moved_output
        report = _extract_facts(output_dir, metadata)
        report_path = output_dir / "_客观事实抽取报告.md"
        report_path.write_text(report.rstrip() + "\n", encoding="utf-8")
        store.update_paper(
            paper_id,
            status="review",
            stage="待讨论标签、关注点与跨文献关系",
            fact_report_path=report_path.relative_to(store.root).as_posix(),
            mineru_output_path=output_dir.relative_to(store.root).as_posix(),
            error=None,
        )
        store.update_task(
            task_id,
            status="completed",
            stage="客观事实报告已生成，等待主对话讨论",
            progress=100,
            error=None,
        )
    except PipelineCancelled as exc:
        store.update_task(task_id, status="cancelled", stage="已停止", error=str(exc))
        store.update_paper(paper_id, status="pending", stage="处理已停止")
    except Exception as exc:
        message = str(exc)[:1000] or exc.__class__.__name__
        store.update_task(task_id, status="failed", stage="处理失败", error=message)
        store.update_paper(paper_id, status="failed", stage="处理失败", error=message)
